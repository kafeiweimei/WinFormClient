...
namespace CoffeeMilk13.DB.DBHelper
{
	public class OracleHelper
    {
        ...
        /// <returns></returns>
        public DataTable ExecuteDataTable(string sql, CommandType commandType, OracleDataReader[] parameters)
        {
            ...
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                using (OracleCommand command = new OracleCommand(sql, connection))
                {
                    ...
                    if (parameters != null)
                    {
                        foreach (OracleDataReader parameter in parameters)
                        {
                            command.Parameters.Add(parameter);
                        }
                    }
                    //通过包含查询SQL的OracleCommand实例来实例化OracleDataAdapter
        ...
        /// <summary>
        /// 执行sql语句及其命令参数返回OracleDataReader
        /// </summary>
        /// <param name="sql">要执行的SQL语句</param>
        /// <param name="commandType">要执行的查询语句的类型，如存储过程或者SQL文本命令</param>
        /// <returns></returns>
        ...
        /// <param name="parameters">Transact-SQL 语句或存储过程的参数数组</param>
        /// <returns></returns>
        public Object ExecuteScalar(string sql, CommandType commandType, OracleDataReader[] parameters)
        {
            object result = null;
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
