...
*		主题：OPCUA帮助类
*	Description：
*		功能：环境搭建测试可参考《https://blog.csdn.net/xiaochenXIHUA/article/details/117254108》
...
namespace CoffeeMilk13.SCADA.OPCUA
{
    public class MyOpcUaHelper
    { 
        #region   基础参数
        //OPCUA客户端
        ...
        /// <param name="connSuccessEvent">连接成功后的事件处理</param>
        public async void OpenConnectOfAnonymous(string serverUrl,Action<bool>connSuccessEvent)
        {
            if (!string.IsNullOrEmpty(serverUrl))
            {
                try
                {
                    ...
                    connSuccessEvent(ConnectStatus);
                }
                catch (Exception ex)
                {
        ...
        /// <param name="connSuccessEvent">连接成功后的事件处理</param>
        public async void OpenConnectOfAccount(string serverUrl, string userName, string userPwd, Action<bool> connSuccessEvent)
        {
            if (!string.IsNullOrEmpty(serverUrl) &&
        ...
        /// <param name="connSuccessEvent">连接成功后的事件处理</param>
        public async void OpenConnectOfCertificate(string serverUrl, string certificatePath, string secreKey, Action<bool> connSuccessEvent)
        {
            if (!string.IsNullOrEmpty(serverUrl) &&
        ...
        /// <summary>
        /// 读取单个节点的历史数据记录(注意：需要节点支持数据记录，否则读取会报错)
        /// </summary>
